...
import { numberTo32ByteHex } from "./utils/stringToHex.ts";
const { provider, networkHelpers } = await network.connect();
const { deployAll } = setupFixtures(provider);
let { env, SimpleTokenReceiver, ERC1363Example, namedAccounts } =
...
describe("ERC1363 合约测试", function () {
  beforeEach(async function () {
    // 在每个测试用例之前重新加载合约和账户
    ({ env, SimpleTokenReceiver, ERC1363Example, namedAccounts } =
      await networkHelpers.loadFixture(deployAll));
  });
  ...
  describe("transferAndCall 功能测试", function () {
    it("transferAndCall 应成功调用 SimpleTokenReceiver 的 onTransferReceived 方法", async function () {
      ...
      await env.execute(ERC1363Example, {
        ...
        args: [SimpleTokenReceiver.address, 10n, data as `0x${string}`],
        account: namedAccounts.deployer,
      });
