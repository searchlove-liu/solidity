...
let { env, TCF1, TCF2, namedAccounts, test_TCF_ERC1155 } =
  await networkHelpers.loadFixture(deployAll);

describe("TCF_ERC1155测试", function () {
  beforeEach(async () => {
    ({ env, TCF1, TCF2, namedAccounts, test_TCF_ERC1155 } =
      await networkHelpers.loadFixture(deployAll));
  });
  describe("mint:对 mint函数 进行测试", function () {
    ...
    it("mint: to 为 0 地址时 revert", async function () {
      await expect(
        env.execute(test_TCF_ERC1155, {
          functionName: "testMint",
          args: [zeroAddress, 1n, 1n, "0x" as `0x${string}`],
          account: namedAccounts.deployer,
        }),
  ...
  describe("balanceOf 与 ownerOf 的安全检测", function () {
    it("balanceOf: account 为 0 地址时 revert", async function () {
      await expect(
        env.read(test_TCF_ERC1155, {
          functionName: "balanceOf",
          args: [zeroAddress, 0n],
        }),
    ...
    it("balanceOf: tokenID 超出范围时 revert", async function () {
      await expect(
        env.read(test_TCF_ERC1155, {
          functionName: "balanceOf",
          args: [namedAccounts.deployer, 10n],
        }),
      ).to.be.rejectedWith("ERR_TOKENID_RANGE");
    });

    it("ownerOf: tokenID 超出范围时 revert", async function () {
      await expect(
        env.read(test_TCF_ERC1155, {
          functionName: "testOwnerOf",
          args: [10n, 0n],
  ...
  describe("approval 与 ESafeTransferFrom 测试", function () {
    it("setApprovalForAll: 不可授权给自己", async function () {
      await expect(
        env.execute(test_TCF_ERC1155, {
          functionName: "setApprovalForAll",
          args: [namedAccounts.deployer, true],
          account: namedAccounts.deployer,
        }),
      ).to.be.revertedWith("APPROVAL_SELF");
    });

    it("ESafeTransferFrom: 未授权调用者无法转移", async function () {
      await env.execute(test_TCF_ERC1155, {
        functionName: "testMint",
        args: [namedAccounts.deployer, 0n, 2n, "0x" as `0x${string}`],
        account: namedAccounts.deployer,
    ...
    it("ESafeTransferFrom: 获得授权的操作员可以转移代币", async function () {
      await env.execute(test_TCF_ERC1155, {
        functionName: "testMint",
        args: [namedAccounts.deployer, 0n, 2n, "0x" as `0x${string}`],
        account: namedAccounts.deployer,
      ...
      expect(
        await env.read(test_TCF_ERC1155, {
          ...
          args: [namedAccounts.deployer, namedAccounts.admin1],
        }),
      ).to.equal(true);
  ...
  describe("接口与未实现函数", function () {
    it("supportsInterface: 支持已声明接口", async function () {
      const erc1155Id = "0xd9b67a26" as `0x${string}`;
      const metadataId = "0x0e89341c" as `0x${string}`;
    ...
    it("safeTransferFrom: 仍未开放默认实现", async function () {
      await expect(
        env.execute(test_TCF_ERC1155, {
          functionName: "safeTransferFrom",
          args: [
      ...
      ).to.be.revertedWith("USE_ESafeTransferFrom");
    });

    it("safeBatchTransferFrom: 尚未实现", async function () {
      await expect(
        env.execute(test_TCF_ERC1155, {
          functionName: "safeBatchTransferFrom",
          args: [
    ...
    it("balanceOfBatch: 尚未实现", async function () {
      await expect(
        env.read(test_TCF_ERC1155, {
          functionName: "balanceOfBatch",
          args: [[namedAccounts.deployer], [0n]],
        }),
      ).to.be.rejectedWith("NOT_IMPLEMENTED");
    });
  });
});
