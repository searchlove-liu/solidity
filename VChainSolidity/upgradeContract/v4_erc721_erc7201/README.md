...
1. 先部署V1合约
2. 再部署代理合约，部署代理合约时，传入V1合约地址和初始化函数数据，直接调用V1的initialize()函数生成就是第二个参数的数据。
   （调用代理合约的InitProxy，第一个参数V1逻辑合约的地址，第二个参数是调用V1逻辑合约initialize函数的input数据，包括V1逻辑合约initialize函数选择器+参数。V1逻辑合约的initialize函数，的作用是赋值owner（必须proxy中调用，如果部署之后调用，黑客会提前调用v1的初始化函数，让合约拥有者变为黑客的账户）因为当前initialize函数是将部署者作为owner，所以不需要传参；`paused`赋值为false；初始化tokenName和tokenSymbol）
3. 代理合约中显示逻辑合约调用函数：将remixIED显示的文件变为V1合约，重新编译。在部署合约功能区域，在`At Address`区域内输入代理合约地址，点击`At Address`就可以在代理合约中显示逻辑合约函数。
